...
bool PlayerUI::IsPlayerInFirst() const
{
    ...
    if(p->getPM()->getCurrLap() > e->getEM()->getCurrLap()){
        return true;
    }
    else if(p->getPM()->getLastCheckPoint() > e->getEM()->getLastCheckPoint()){
        return true;
    }
    else if(p->getPM()->getCurrLap() == e->getEM()->getCurrLap() && p->getPM()->getLastCheckPoint() == e->getEM()->getLastCheckPoint() && p->getPM()->getDistanceFromCheckPoint() < e->getEM()->getDistanceFromCheckPoint()){
        return true;
    }
    else if(p->getPM()->getLastCheckPoint()==-1 && e->getEM()->getLastCheckPoint()>0 &&
            p->getPM()->getCurrLap() == e->getEM()->getCurrLap()){
        return true;
    }
	return false;
}
